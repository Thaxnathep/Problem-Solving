...
class MusicPlaylist:
    ...
    def display_playlist(self):
        ...
        while current:
            ...
            playlist_songs.append(f"{marker}{count}. {current.title} by {current.artist}")
            current = current.next_song
            count += 1
    ...
    def play_current_song(self):
        if self.current_song:
            ...
            if self.current_song.audio_file:
                ...
                st.warning("No audio file attached to this song.")
        else:
...
st.title("🎶 Music Playlist App (Playable)")
...
uploaded_file = st.sidebar.file_uploader("Upload Audio File", type=['mp3', 'wav', 'ogg'])
...
if st.sidebar.button("Add Song to Playlist"):
    if new_title and new_artist and uploaded_file:
        ...
        st.sidebar.warning("Please enter title, artist AND upload an audio file.")
...
clicked_prev = col1.button("⏪ Previous")
clicked_play = col2.button("▶️ Play / Reload")
clicked_next = col3.button("⏩ Next")
...
st.markdown("### Now Playing Widget")
